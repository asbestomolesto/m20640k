...
void do_new(FILE **file,int argc,char **argv)
{
    ...
    imgbuf = malloc(286720);  /* total image size: 1120 blocks 
                                must be exact to create a new 320K disk -asb */
    if (! imgbuf) {
        fprintf(stderr,"do_new: cannot allocate memory: %s\n",strerror(errno));
        return;
    }

    memset(imgbuf,0,286720); /* Write 286720 bytes with 0 in that buffer. this must be changed for 640k. DOUBLE? */
    memset(imgbuf+0x1000,0xff,0x100); /* write 0x100 times FF in the buffer at address 0x1000 */
    memset(imgbuf+0x20200,0xff,0x21000-0x20200); /* fill FF in that buffer starting from 0x20200 for BOH bytes */
    /* this shit above seem to initialize the new disk */
    *(imgbuf+0x2001c) = 7;
    *(imgbuf+0x2001d) = 0x32; /* those 2 bytes I found them at 0x2001C/0x2001D in newdisk.img */
    *(imgbuf+0x20038) = 0xff;
    *(imgbuf+0x20039) = 0xff; /* same as above, at 0x20038 / 0x20039 */
    for (j=3,i=0x202f0; i<0x20ff0; i+=0x100,j++) {
        *(imgbuf + i + 0xc) = 0;
        *(imgbuf + i + 0xd) = 0;
        *(imgbuf + i + 0xe) = 0;
        *(imgbuf + i + 0xf) = j;
        /* Prepare the directory area, there are 14 blocks of 0x100 bytes. 
           I found those things in the dumps. Counting from 00 00 00 03
           to 00 00 00 0F. all FF between them. 
           Please note: 640k has 28 blocks! How to do this? IDK because dir jumps around tracks -asb */
    }
    if (fwrite(imgbuf,286720,1,*file) != 1) { /* this must be changed for 640k. DOUBLE? /*
...
static void do_put_one_file(FILE **file,char *srcfile,char *destfile)
{
    ...
    if (pcos_destfile_p) {
        if (*(pcos_destfile_p+1)) {
            strncpy(pcos_destfile,pcos_destfile_p+1,14); /* file name lenght? */
        }
        else {
    ...
    /* get block map block */
    if (read_phys_block(*file,0x200,(void *)dskhdr)) { /* main block address 0x20200 */
    ...
    for (found=-1,i=0; i<diskdir->entries; i++) {
        if (!strncmp(diskdir->files[i].name,pcos_destfile,14)) { /* file name lenght? /*
            found = i;
            break;
        }
    }
    ...
    if (found != -1) {
      ...
      finishname:
        ...
        strncpy(blkbuf.files[index].name,pcos_destfile,14); /* file name lenght? */
        goto finishname;
    }
    /* write back the modified block */
    if (write_phys_block(*file,0x200,(void *)dskhdr)) { /* 0x20200 start of main disk block */ 
...
void do_rm(FILE **file,int argc,char **argv)
{
    ...
    if (read_phys_block(*file,0x200,(void *)dskhdr)) { /* start of main disk block 0x20200 */
    ...
    /* write back the modified block */
    if (write_phys_block(*file,0x200,(void *)dskhdr)) { /*  the fucking main disk block start at 0x20200 */
