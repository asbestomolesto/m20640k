                      ...
                      struct dir_sector *blockbuf)
{
    ...
    *filephysflag = 0;
    ...
    for (i = 0;  i < PCOS_NUM_DIR_BLOCKS; i++) {  /* all directory blocks. PCOS_blablah is 14, 640K is 28!!! */
        /* read block */
        if (read_phys_block(floppy,*blknum = 0x202 + i,(void *)blockbuf)) {/* read 14*0x100 blocks from 0x20200
                                                                            to find free dir entries! */
...
/* --- same as above, simple block copy --- */
        for (*index = 0; *index < PCOS_DIR_ENTRIES_BLOCK; (*index)++) {  /* loop over all entries in this block 
                                                                            PCOS_DIRblabla is 14 640k is 28 */
            if (blockbuf->files[*index].name[0] == 0xff) {
                  ...
                  struct dir_sector *blockbuf)
{
    ...
    *filephysflag = 0;
    ...
    for (i = 0;  i < PCOS_NUM_DIR_BLOCKS; i++) {  /* all directory blocks */
        /* read block */
        if (read_phys_block(floppy,*blknum = 0x202 + i,(void *)blockbuf)) { /* 0x20200 in the dump */
        ...
        for (*index = 0; *index < PCOS_DIR_ENTRIES_BLOCK; (*index)++) {  /* loop over all entries in this block */
            if (blockbuf->files[*index].name[0] == 0xff) {
                /* if 2nd char also is 0xff, end of used directory structure has been reached
                   that means we can add a file here, it's free space in the dir structure. -asb */
...
int get_dir(FILE *floppy,struct m20dir **m20dir)
{
    ...
    *m20dir = malloc(dirsize);   /* allocate space for directory table */
    ...
    for (i = 0;  i < PCOS_NUM_DIR_BLOCKS; i++) {  /* all directory blocks */
        /* read block */
        if (read_phys_block(floppy,0x202 + i,(void *)&blockbuf)) { /* 0x20200 as usual */
            fprintf(stderr,"get_dir: cannot read dir block\n");
            return(-1);
        }
        for (j = 0; j < PCOS_DIR_ENTRIES_BLOCK; j++) {  /* all files in 1 block. that PCOS_ is 14, 640K is 28. */
...
int get_file(FILE *floppy,struct m20dir_entry *srcfile,FILE *destfile)
{
    ...
    transbuf = malloc(300000); /* just to be sure? -asb */ 
    if (! transbuf) {
